...
#
# v.9b - store all pics, wait till end, then save
#        all together. Sequences are identical.
...
# First obtain all the images, plot none.
smpls=[]
for i in range(n_plots):
  mset=runtest(sess, batch, i)
  smpls.append(mset)

# Now plot all the images at the same time.
for i in range(n_plots):
  ...
  for bat in range(n_bats):
    ...
    mimg=[]
    for imn in range(batch_size*2):
      mimg.append(smpls[i][bat][imn])

    m=montage_2(mimg)
    axs[bat].imshow(m)
    axs[bat].set_title("batch #"+str(bat))

  plt.savefig("tmp/y9b_"+str(i)+"_"+TID+".png", bbox_inches="tight")
