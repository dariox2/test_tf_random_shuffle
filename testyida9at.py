...
#
# v.9at - 2 figures with 3 batches each, plotting
#        at the same time, fails to repeat the
#        same pseudorandom sequence.
#        Removed montage; keeps failing.
#        Removed fig, axs; keeps failing.
#        Removed pyplt; diff text; keeps failing.
#        Changed to only 1 run batch;
#        added code to close coordinator gracefully;
#        keeps failing.
#
...
def runtest(sess, batch, idt):
  ...
  for bat in range(n_bats):
    ...
    for imn in range(batch_size):
      ...
      print("y",imn,bat,idt,img1[50][50:51])
